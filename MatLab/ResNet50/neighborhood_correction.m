...
% MODIFICATO cancello \ prima di cells
opts.dataDir = fullfile(opts.rootPath, 'cells');
opts.imdbPath = fullfile(opts.expDir, 'imdb.mat');
opts.numWords = 8 ;
opts.numDescrsPerWord = 3000 ;

% MODIFICATO a 0 per la non presenza di una GPU dedicata
...
idx5c = net.getVarIndex('res5c_branch2a'); %indice del layer a 164
...
% MODIFICATO da gpu a cpu per mio test
if useGPU == 0
    net.move('cpu');
else 
    net.move('gpu');
...
if exist(sprintf('feats-res%s-cv%s_%s.mat', num2str(opts.whichresnet), num2str(opts.trainset), num2str(opts.testset)),'file')
    ...
    for idx = 1:numel(testID)
        ...
        % MODIFICATO
        % All'inizio era 'inputs = {'data', gpuArray(im_resized)}', tolgo tale
        % funzione perchè non posso utilizzare la gpu, così da averlo
        % direttamente in memoria locale: modificato per mio test
        if useGPU == 0
            inputs = {'data', im_resized} ;
        else 
