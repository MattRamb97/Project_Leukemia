...
def getPyRadImageAndMask(grayScaleImage, jointSeries=False):
    ...
    #We updated python version from 3.9 to 3.10 in spyder Anaconda
    if jointSeries:
...
def computeDCTransform(imgPath, noOfFeatures=1024):
    ...
    #Take a look to https://stackoverflow.com/questions/50445847/how-to-zigzag-order-and-concatenate-the-value-every-line-using-python
...
def createAugmPatLvDivDataframe():
    ...
    p0 = mp.Process(name='train_AugmPatLvDiv', target=createTrainDataframe)
    p1 = mp.Process(name='valid_AugmPatLvDiv',target=createValidDataframe)
    p2 = mp.Process(name='test_PatLvDiv',target=createTestDataframe)
    p0.start()
    p1.start()
    p2.start()
    p0.join()
    p1.join()
    p2.join()
    createTrainDataframe()
    createValidDataframe()
    createTestDataframe()
    
if __name__ == '__main__':
    p0 = mp.Process(name='AugmPatLvDiv', target=createAugmPatLvDivDataframe)
    p0.start()
    p0.join()
    createAugmPatLvDivDataframe()
    print(f"\nEnd Script!\n{'#'*50}")
